...
public class Game extends Thread {
    ...
    public Game(GameDTO gameDTO, Bot botA, Bot botB) {
        this.rows = gameDTO.rows;
        this.cols = gameDTO.cols;
        this.inner_walls_count = gameDTO.inner_walls_count;
        this.g = gameDTO.g;
        ...
        playerA = new Player(
                gameDTO.playerDTOA.userId,
                botIdA,
                typeA,
                botCodeA,
                gameDTO.playerDTOA.sx,
                gameDTO.playerDTOA.sy,
                gameDTO.playerDTOA.steps);
        playerB = new Player(
                gameDTO.playerDTOB.userId,
                botIdB,
                typeB,
                botCodeB,
                gameDTO.playerDTOB.sx,
                gameDTO.playerDTOB.sy,
                gameDTO.playerDTOB.steps);
    }
    ...
    public Integer getInner_walls_count() {
    ...
    public void run() {
        for (int i = 0; i < 1000; i ++) { //1000步之内游戏肯定结束
            ...
            if (Thread.currentThread().isInterrupted()) {
                // 处理线程中断逻辑，如清理资源等
                System.out.println("Thread is interrupted, stopping old game-Thread.");
                break; // 退出循环，结束线程执行
            }
    ...
    private boolean nextStep() { // 等待两名玩家下一步操作
        //由于前端动画200ms才能画一个格子(蛇是每200ms走一步)
    ...
    private void sendResult() {
        ...
        WebSocketServer.delGameFromRedis(playerA.getId());
        if (!Objects.equals(playerB.getId(), WebSocketServer.AI.getId())) {
            WebSocketServer.delGameFromRedis(playerB.getId());
        }
    }
    ...
    private void sendMove() {
        ...
        try {
            ...
            GameDTO gameDTO = new GameDTO(
                    this.rows,
                    this.cols,
                    this.getInner_walls_count(),
                    this.g,
                    this.getPlayerA(),
                    this.getPlayerB());
            WebSocketServer.saveGameToRedis(playerA.getId(), gameDTO);
            if (!Objects.equals(playerB.getId(), WebSocketServer.AI.getId())) {
                WebSocketServer.saveGameToRedis(playerB.getId(), gameDTO);
            }
        } finally {
            lock.unlock();
        }
